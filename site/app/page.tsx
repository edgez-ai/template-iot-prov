  ...
  return <main className="shell">
    ...
    {!user ? <section className="auth-grid">
      ...
      <div className="columns">
        <form className="panel" onSubmit={addDevice}><h2>Register an Appwrite device</h2><label>Unique serial<input value={serial} onChange={(e) => setSerial(e.target.value)} pattern="[A-Za-z0-9][A-Za-z0-9._:-]{0,35}" maxLength={36} placeholder="test123" required /></label><label>Display name<input value={name} onChange={(e) => setName(e.target.value)} maxLength={128} placeholder="Workshop sensor" required /></label><button disabled={busy}>Create device + MQTT credential</button>{credential && <div className="credential"><small>COPY MQTT CREDENTIALS NOW</small><code>clientId: {credential.clientId}</code><code>username: {credential.username}</code><code>password: {credential.password}</code><p>Publish to projects/{projectId}/devices/{credential.username}/telemetry/&lt;channel&gt;</p></div>}</form>
