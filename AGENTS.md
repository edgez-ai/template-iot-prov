...
| `function/` | Appwrite Function; the only TablesDB reader/writer. |
...
Keep these boundaries. Never expose `APPWRITE_API_KEY`, claim-code hashes, or
device-token hashes to clients. Clients must not access TablesDB directly.

## API contract

- `GET /health`: liveness response.
- `POST /mqtt`: trusted broker-adapter webhook. Requires `x-mqtt-secret` and
  accepts `{ "topic": "devices/<deviceId>/telemetry", "payload": {...} }`.
